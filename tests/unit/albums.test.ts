...
describe("album loaders", () => {
  ...
  it("returns the first featured album and falls back when no featured records exist", async () => {
    mockFetch.mockResolvedValueOnce([PLACEHOLDER_ALL_ALBUMS[1]]);
    await expect(getFeaturedAlbum()).resolves.toEqual(PLACEHOLDER_ALL_ALBUMS[1]);

    mockFetch.mockResolvedValueOnce([]);
    await expect(getFeaturedAlbum()).resolves.toEqual(PLACEHOLDER_FEATURED_ALBUMS[0]);
  });
  ...
  it("returns null neighbors when the album slug is missing from the album list", async () => {
    mockFetch
      ...
      .mockResolvedValueOnce(PLACEHOLDER_ALL_ALBUMS.filter((album) => album.slug.current !== "march-madness"));
